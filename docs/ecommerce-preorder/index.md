...
Every pre-order moves through 9 defined statuses: Requested → Accepted → Prepayment Requested → Prepayment Confirmed → Final Pre-Order → In Shipping → Delivered (plus Cancelled and Refunded at any eligible stage). You control each transition from the admin panel.
