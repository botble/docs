...
Carento uses Expo's versioning, not `pubspec.yaml`. There are two numbers to
keep in mind:

- **`version`** — the public, human-readable version shown in the stores
  (Major.Minor.Patch, e.g. `1.2.0`).
- **Build number** — an internal counter that must increase for every store
  upload: iOS `buildNumber`, Android `versionCode`.
...
With this, EAS stores the current build number on its servers and bumps it by 1
for every `eas build --profile production`. You do not manage `buildNumber` /
`versionCode` by hand — just bump `APP_VERSION` when the public version changes.
...
```js
ios: {
  bundleIdentifier: "com.carento.mobile",
  buildNumber: "5",           // iOS build number (string)
  // ...
},
android: {
  package: "com.carento.mobile",
