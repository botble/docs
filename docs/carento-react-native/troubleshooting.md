...
Common issues when setting up and running the Carento (Expo) app, and how to fix them.
...
## Blank screens / no data — wrong or missing `API_BASE_URL`

The most common cause of empty screens is a bad backend URL.

1. Open `.env` and confirm `API_BASE_URL` points at your running Carento (Botble) backend, e.g. `http://carento.test` — **no** trailing slash and **no** `/api/v1` suffix (it is appended automatically).
2. Open `http://carento.test/api/v1/car-manager/cars` in a browser. You should get JSON with a `data` array of cars.
...
## iOS build fails with code-signing error (no certificate)
...
## Stale bundle / old code after changes — clear the Metro cache
...
## Empty screens even though the backend is reachable

If the app connects but a section is empty, the data (or its relations) is missing on the backend, not in the app:

1. Confirm the backend has published/available cars, dealers (vendor customers), locations, and blog posts.
2. Confirm the car-manager plugin is installed and enabled.
3. Confirm required relations are set (a car needs a make/type, images, and a location to render fully).
