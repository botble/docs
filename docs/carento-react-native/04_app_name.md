...
The app name is **never hardcoded** in any UI string — every translation that shows
it uses the `{{appName}}` variable, which is filled from `APP_NAME` at runtime
(registered globally as an i18next interpolation default in `src/i18n/index.ts`):

```json
"auth": { "loginTitle": "Sign in to {{appName}}" }
```

So changing `APP_NAME` rebrands the app name across **all** screens and **all four
languages** at once — no string edits needed. When adding your own strings that need
the app name, write `{{appName}}` (not the literal name) and it resolves automatically.
