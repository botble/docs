...
A step-by-step guide covering everything from initial setup to publishing your Carento React Native (Expo) app on the Apple App Store and Google Play Store, and connecting it to your Botble car-rental website so cars and bookings sync automatically.
...
- [ ] Cars appear on the home screen
- [ ] Search and filters return cars from your site
- [ ] Car detail shows specs, photos, and reviews
- [ ] You can log in with an account from your website
- [ ] The booking flow calculates a price
- [ ] Checkout opens the hosted payment page in a WebView
- [ ] Favorites save

**If cars appear, your app is connected to your website.**

---

## Part 3: How Cars & Bookings Sync Automatically

The app connects to your Botble website through its **REST API** (`car-manager` plugin):

```
Your Botble Website (yourcarento.com)
        ↓
    REST API (/api/v1/car-manager)
        ↓
    Carento app fetches data
        ↓
    Cars, dealers, bookings, blog displayed
```
...
|---|---|---|
| **Cars** | Real-time on each visit | `/api/v1/car-manager/cars` |
| **Search / filters** | Real-time | `/api/v1/car-manager/cars/search`, `/cars/filters` |
| **Car detail** | Real-time | `/api/v1/car-manager/cars/{slug}` |
| **Bookings** | Real-time when the customer views | `/api/v1/car-manager/bookings` |
| **Pricing** | Live calculation | `/api/v1/car-manager/calculate-price` |
| **Favorites** | Synced with the account | `/api/v1/car-manager/favorites` |
| **Taxonomy** (makes, types, fuels…) | Real-time | `/api/v1/car-manager/car-makes`, etc. |
| **Blog** | Real-time | `/api/v1/posts` |
| **User accounts** | Shared login with the website | `/api/v1/car-manager/auth/login` |

- **Add a car on your website** → it appears in the app automatically
- **Update a price** → the app shows it immediately
- **Customer books in the app** → the booking appears in your admin panel
...
The defaults in `app.config.js` are `com.carento.mobile` (both iOS `bundleIdentifier` and Android `package`) and the slug `carento-mobile`. Change them to your own before publishing:
...
- [ ] Data safety form (personal info for accounts/bookings; payment handled via web checkout)
...
- [ ] Verify cars load, search works, and car detail opens
- [ ] Test the full booking flow (dates → extras → details → review → checkout)
...
2. TEST LOCALLY
   ...
   └── Verify cars, login, booking, checkout
...
The `POST /car-manager/bookings` endpoint creates a **PENDING** booking without a payment URL. Checkout is completed on the backend's **hosted payment page**, loaded in a WebView (`app/checkout-webview.tsx`). This means every Botble payment gateway works without a native SDK.
