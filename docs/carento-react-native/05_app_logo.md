...
Carento's app icon, adaptive icon, splash image, and web favicon are all configured in `app.config.js` and point at files in the `assets/` folder. To rebrand, replace the asset files (keeping the same filenames) and regenerate the native projects.
...
The adaptive icon and splash backgrounds default to the Carento primary (`#84cc16`). Override the splash background with `SPLASH_BACKGROUND_COLOR` in `.env` (hex **with** `#`); the adaptive icon background is set directly in `app.config.js`.
