...
Carento is a React Native (Expo) mobile app that connects to a Botble car-rental backend running the **car-manager** plugin. One codebase builds both iOS and Android. Cars, bookings, dealers, and blog posts are managed entirely from your Botble admin and appear in the app in real time.
...
- **Booking flow** — 4-step flow (dates → extras → details → review) with live price calculation and coupons
