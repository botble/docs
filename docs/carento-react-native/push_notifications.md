...
Carento sends push through Firebase Cloud Messaging. The app registers an FCM registration
...
- An **Android** app with package name `com.carento.mobile`. Download `google-services.json`.
- An **iOS** app with bundle ID `com.carento.mobile`. Download `GoogleService-Info.plist`.
