...
app/                          expo-router routes (screens — thin, delegate to hooks/services)
  ...
  car/[slug].tsx              car detail
  car/[slug]/book.tsx         4-step booking flow
  ...
  blog/, dealer/[id].tsx, favorites.tsx, onboarding.tsx, ...
...
src/
  ...
  components/{cars,home,search,booking,dealers,account,auth,blog,bookings}/
  hooks/                      useDebounce, useBiometric, useSocialAuth, useDealers, usePushNotifications
...
export async function fetchCarBySlug(slug: string): Promise<Car> {
  const res = await api.get(`/car-manager/cars/${slug}`);
  return res.data;
}
```
