...
This guide sets up **Google Sign-In** in the Carento React Native (Expo) car-rental app. On the login screen it renders the **Google** button in the "or continue with" row.
...
|---|---|---|
| iOS bundle ID | `com.carento.mobile` | `ios.bundleIdentifier` in `app.config.js` |
| Android package | `com.carento.mobile` | `android.package` in `app.config.js` |
