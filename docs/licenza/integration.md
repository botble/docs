...
This guide explains how to integrate Licenza into your software to enforce license activation and verification.
...
Licenza uses a 3-step flow:

1. **Activate** — User enters their license code on first install. Your software sends it to Licenza and receives encrypted license data.
...
1. **Licenza server** deployed and accessible (e.g., `https://license.yourdomain.com`)
2. **External API Key** — created in **Licenza → Settings → API Keys**
3. **Product** — created in **Licenza → Products** with a reference ID (e.g., `PROD-001`)
4. **License** — created in **Licenza → Licenses** with a license code
...
Licenza handles all validation logic server-side. However, your client-side code (the API client and verification logic above) could be reverse-engineered to bypass the license check — especially with the advancement of AI tools.

**Your responsibility** is to protect the client-side code that calls the Licenza API. Here are recommended tools based on your tech stack:
...
::: tip Key Point
Regardless of the encoder you choose, the most important protection is already in place — Licenza validates everything server-side. Your client code just needs to be hard enough to tamper with so casual users won't bother. Encoding makes reverse-engineering impractical for most people.
:::
...
- Verify the Licenza server URL is correct and accessible
...
- Confirm the license code exists in **Licenza → Licenses**
