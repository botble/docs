...
1. Go to **Licenza → Settings → General**
2. Enable **Webhooks**
...
The **Webhook URL** is the endpoint on your external server where Licenza sends HTTP POST requests when license events occur. This is a "push" notification system — instead of your application polling Licenza for changes, Licenza proactively notifies your server.
...
The **Webhook Secret** is an optional (but strongly recommended) shared secret key that Licenza uses to **sign** each webhook payload. This lets your receiving server verify that the request genuinely came from Licenza and hasn't been tampered with.

**How it works:**

1. You set a secret string in Licenza settings (e.g., `my-super-secret-key-123`)
2. On every webhook dispatch, Licenza computes an HMAC-SHA256 signature over the JSON payload using this secret
...
- Store it securely on both Licenza and your receiving server
...
Every webhook dispatch attempt is recorded in the **Activity Logs** (Licenza → Activity Logs):
