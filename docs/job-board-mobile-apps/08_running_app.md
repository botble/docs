...
This runs `scripts/run-ios-sim.sh`, which builds with ad-hoc signing (`CODE_SIGN_IDENTITY="-"`) and installs straight onto the iOS Simulator. Ad-hoc signing requires no Apple certificate while still applying the app's entitlements (keychain access for biometric auth, Apple Sign-In). It's the best choice when you have no Apple developer certificate. The script boots the named simulator, starts Metro if it is not already running on port 8081, discovers the workspace name from `ios/`, builds it with `xcodebuild`, then installs and launches the app.
