...
Most setup problems with JobHive are **not** in the app. The app is a thin REST client: if a screen is empty, an image is broken or a request is rejected, the cause is almost always something on the Laravel job-board backend that has not been configured yet.
...
```
https://your-site.com/api/v1/jobs
```
...
| Endpoint not found | `404` | Job-board plugin not activated, or `/api/v1` missing from the route list |
...
**Cause:** the Botble REST API is switched off. The backend returns `503` with `error: API_DISABLED`, and the app shows its maintenance gate.
...
## 2. Job-board plugin not activated (`404`)

**Symptom in the app:** the app starts, but job, company and taxonomy screens fail while generic endpoints may still work.

**Cause:** the job-board plugin is not active on the backend, so its API routes are never registered.

**Fix:**

1. Go to **Admin → Plugins**.
2. Make sure the **Job Board** plugin is **activated**.
3. Reload `https://your-site.com/api/v1/jobs` in a browser. A `404` here after activation means the route cache is stale, so clear the cache from **Admin → Platform Administration → Cache Management**.

::: warning This app only works with a job-board backend
JobHive is built against the job-board plugin's API, which ships with **Jobcy** and **JobBox**. Pointing `API_BASE_URL` at an ecommerce, real-estate or plain CMS site will produce `404` on every listing endpoint. See [Overview](overview.md) for the backend requirement.
:::
...
**Cause:** a bot-protection layer in front of the backend is rejecting a non-browser client. The app identifies itself with a descriptive User-Agent built from your app name and version, such as `JobHive/1.0.0 (ios 18.0)`, which some default Cloudflare, WAF or ModSecurity rules treat as an unknown bot. This is not hypothetical: the public demo backend returns `403` to requests carrying no recognisable User-Agent at all.
...
```bash
curl -i -H "User-Agent: JobHive/1.0.0 (ios 18.0)" \
...
**Symptom in the app:** job listings load with text and salaries, but every company logo is a grey placeholder. Invoice PDFs and the package checkout may also fail to open.
...
3. Re-open `https://your-site.com/api/v1/jobs` and check one company `logo` value in the JSON. Paste that URL into a browser: it must load the picture directly.
...
**Symptom in the app:** no error, no maintenance screen, just an empty list with a "no results" state.

**Cause:** the backend answered correctly and the answer was genuinely empty. The app cannot invent data.

Work through these in order:

1. **Nothing is published.** Jobs must be **Published** *and* approved. Draft, pending, expired or rejected postings are excluded from the API.
2. **The job has expired.** Job-board listings carry an expiry date, and expired postings drop out of the public feeds even though they still appear in the employer's own dashboard.
3. **Required relations are missing.** A job needs a category, job type and a company to render fully. A job with no location never appears in the map search.
4. **Companies are missing.** Companies belong to employer accounts. If none are approved, the companies directory is empty by design.
5. **Taxonomies are empty.** The filter sheet is built from `/job-types`, `/categories`, `/job-skills`, `/career-levels`, `/functional-areas`, `/job-shifts` and `/job-experiences`. Seed these under **Admin → Job Board** or the filters render with no options.
6. **The requested language has no translation.** See the next section.
7. **The requested currency is not configured.** See section 9.

Confirm by comparing counts: what **Admin → Job Board → Jobs** shows against what `https://your-site.com/api/v1/jobs` returns.
...
```bash
curl -H "X-LANGUAGE: ar" https://your-site.com/api/v1/jobs
```
...
## 9. Currency not configured: wrong or unconverted salaries

**Symptom in the app:** salary figures do not change when the user picks another currency, or the currency list in settings is empty.
...
**Cause:** the provider handshake succeeded, but the backend could not complete it. The app posts the provider token to `POST /api/v1/auth/{provider}`.
...
## 12. Employer package checkout fails in the WebView

**Symptom in the app:** an employer taps a credit package, the checkout WebView opens, and it shows a login screen, a blank page, or a "no payment method available" error.

**Cause:** the checkout is your backend's own hosted checkout page, rendered in a WebView. The app calls `POST /api/v1/account/packages/{id}/subscribe` and hands off to the URL it returns, so anything wrong there is a backend configuration issue.
...
3. **Check `APP_URL` again.** The WebView URL is built from your site root; a wrong `APP_URL` sends the employer to the wrong host, which is why the session looks logged out.
...
5. **Confirm packages exist and are published** under **Admin → Job Board → Packages**.
...
**Cause:** delivery is sent by the backend through Firebase Cloud Messaging. The app only registers its device token, via `POST /api/v1/device-tokens` on sign-in.
...
## 14. Uploads fail: resumes, cover letters and company logos

**Symptom in the app:** applying to a job works until a resume or cover letter is attached, then the submit fails, often after a long wait. The same happens when an employer uploads a company logo or cover image.
...
3. Check the media rules under **Admin → Settings → Media**, which cap file size and allowed extensions independently of PHP. Resume uploads need `pdf`, `doc` and `docx` on the allowed list, and CVs are frequently larger than image uploads.
...
**Symptom in the app:** a long spinner, then a request-failed message. It usually affects the heaviest screens first, such as the map search or a large job list.
...
4. Reduce `MAP_PIN_LIMIT` in the app's `.env` (default `200`) if the map query is the slow one.
...
- The exact URL you tested and the full response, from `curl -i https://your-site.com/api/v1/jobs`
