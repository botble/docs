...
JobHive keeps the branded splash on screen a little longer than the OS would, so the first frame users see is your styled home screen. There's never a white flash of unstyled content while fonts and cached settings load.
...
- **Image**: `assets/splash-icon.png`.
- **Background**: The `SPLASH_BACKGROUND_COLOR` environment variable, defaulting to the brand primary `#5749CD`.
