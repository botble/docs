...
JobHive ships production builds and submits them to the stores with **EAS** (Expo Application Services). You do not run `xcodebuild`, keystore tooling, or Xcode archives by hand. EAS builds in the cloud and manages your signing credentials. There is no `flutter build` step here.
