...
2. Open **Identifiers** and select your app's App ID (or create one that matches `com.botble.jobboard`).
...
2. Enter a description and an identifier (e.g. `com.botble.jobboard.service`).
...
| Backend rejects the token | Verify the Services ID, Team ID, Key ID, and `.p8` in Botble match the Apple Developer portal, and that the bundle ID is `com.botble.jobboard`. |
