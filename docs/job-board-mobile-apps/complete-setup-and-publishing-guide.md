...
- [ ] App Privacy answers (contact info for accounts/applications, purchase history)
- [ ] **A demo account** for the reviewer (email + password) and reviewer notes: "Job-board app connecting to yoursite.com; checkout is completed on the hosted web page in a WebView"
...
- [ ] Data safety form (personal info for accounts/applications; payment handled via web checkout)
...
### Do I need to manually add jobs to the app?

**No.** The app pulls all jobs automatically from your Botble website via the API. Manage jobs in admin; the app displays them in real time.
...
The candidate flow has **no payment step** — applying to a job submits a free application (`POST /api/v1/jobs/{id}/apply`). The only in-app payment is an **employer** buying a credits package: the backend returns a `checkout_url` that is opened on its **hosted payment page** in a WebView (`app/employer/checkout-webview.tsx`). This means every Botble payment gateway works without a native SDK.
