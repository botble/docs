...
::: warning
This requires tax to be enabled in `Ecommerce` -> `Settings` -> `Tax`. See the ecommerce tax documentation for how zones and rates work.
:::
...
- **Commission settings are ignored** while in subscription mode, including category-based commission and per-vendor overrides.
