...
Verify Gumroad license keys during activation. When a customer activates your software with their Gumroad license key, License Manager calls Gumroad's API in real time, rejects refunded/chargebacked/cancelled purchases, and auto-creates a local `ProductLicense` so subsequent verifications are served from the local database — no further Gumroad calls.
...
4. A **License Manager server** with an external API key configured.
...
Log into the License Manager admin panel:

**Settings → License Manager → General → Gumroad Integration**

| Field | Required | Description |
|---|---|---|
| **Enable Gumroad Integration** | Yes | Master toggle. When off, the Gumroad Product ID field is hidden on the Product form. |
| **Gumroad Access Token** | No | Personal access token with `edit_products` scope. Only needed if you want License Manager to remotely **disable** a license on Gumroad. Public license **verification** does NOT require a token. Create one at [gumroad.com/settings/advanced](https://gumroad.com/settings/advanced). |
...
**License Manager → Products → your product → Edit**
...
If you sell the same software on Gumroad and (say) Envato, create ONE local Product and fill in both `envato_id` and `gumroad_product_id`. License Manager picks the right provider based on the `verify_type` the client sends.
:::

## Client-side: activating a license

Your client app sends the user's Gumroad license key to your License Manager server. Set `verify_type` to `gumroad`:
...
Instead of waiting for the customer's first activation to call Gumroad's verify API, you can have **Gumroad push every sale** to License Manager. Each sale creates a local `ProductLicense` row immediately, so:
...
- You see every Gumroad sale in your License Manager dashboard.

### 1. Set a webhook secret in License Manager

**Admin → Settings → License Manager → General → Gumroad Integration → Gumroad Ping Webhook Secret**
...
```
https://<your-license-manager-domain>/api/external/webhooks/gumroad/<your-secret>
```
...
| Scenario | License Manager response |
|---|---|
...
Gumroad's ping does NOT sign requests (no HMAC). Authentication is via the secret embedded in the URL path. License Manager uses a timing-safe `hash_equals()` comparison. To rotate the secret, change it in admin AND update the Gumroad ping URL — keep both in sync.
...
When you mark a license as invalid in the admin panel, License Manager only deactivates LOCAL activations by default. To also push **disable** to Gumroad (so the same key cannot be reused), set the **Gumroad Access Token** in settings, then call from a tinker shell:
...
5. **Network**: the License Manager server cannot reach `api.gumroad.com`. From SSH: `curl -I https://api.gumroad.com/`.
...
License Manager calls these Gumroad endpoints:
...
No, not for verification. The token is only needed if you want License Manager to remotely disable license keys on Gumroad.
...
Yes — turn off **Increment Gumroad Uses Counter** in settings. License Manager still tracks activations locally; Gumroad's per-license counter just stays at zero.
