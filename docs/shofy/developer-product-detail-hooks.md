...
Shofy provides hook filters on the product detail page that allow plugins to inject custom content without modifying theme files. All hooks use `apply_filters()` and pass the `$product` model as context.
