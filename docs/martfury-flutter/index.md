# MartFury — Flutter E-commerce App for Botble

A Flutter mobile app that connects to a [Botble e-commerce backend](https://codecanyon.net/item/martfury-multipurpose-laravel-ecommerce-system/29925223). One codebase builds Android and iOS.

## Get started

1. [Overview](overview.md)
2. [Enable the API on your website](backend-api-setup.md)
3. [Installation](installation.md)
4. [License activation (purchase code)](license-activation.md)
5. [Configuration](configuration.md)
6. [Complete Setup & Publishing Guide](complete-setup-and-publishing-guide.md)

## Customize the app

| Topic | Guide |
|---|---|
| Colors | [01_theme_colors.md](01_theme_colors.md) |
| Font | [02_app_font.md](02_app_font.md) |
| App name | [04_app_name.md](04_app_name.md) |
| Logo and icons | [05_app_logo.md](05_app_logo.md) |
| API URL & key | [06_api_base_url.md](06_api_base_url.md) |
| Translations | [07_translations.md](07_translations.md) |
| Ad keys | [03_ad_keys.md](03_ad_keys.md) |
| Profile links | [11_profile_links.md](11_profile_links.md) |
| Splash screen | [17_splash_screen.md](17_splash_screen.md) |
| Loading screen | [18_loading_screen.md](18_loading_screen.md) |

## Build & deploy

- [Running the app](08_running_app.md)
- [Deploying to stores](09_deploying_app.md)
- [Version management](10_version_management.md)

## Social login

- [Google](14_google_login_setup.md)
- [Apple](13_apple_login_setup.md)
- [Facebook](15_facebook_login_setup.md)
- [Twitter / X](12_twitter_login_setup.md)
- [Enable / disable providers](16_social_login_configuration.md)

## Push notifications

- [Firebase Cloud Messaging setup](fcm_setup.md)

## Feature list

- Product browse, search, categories, brands, flash sales, recently viewed
- Authentication (email/password and social)
- Cart, coupons, wishlist, product comparison
- WebView checkout (compatible with all backend payment gateways: Stripe, PayPal, Razorpay, Mollie, SSLCommerz, COD, etc.)
- Order history and tracking
- 29 built-in languages, RTL support
- Light / dark / system theme
- Push notifications via Firebase
- Maintenance mode

## Reference

- [API Integration](api-integration.md)
- [Development Guide](development.md)
- [Upgrade Guide](upgrade.md)
- [Troubleshooting](troubleshooting.md)
- [FAQ](faq.md)
- [Support](support.md)
- [Release notes](releases.md)

## Resources

- Documentation: https://docs.botble.com/martfury-flutter
- Backend demo: https://martfury.botble.com
- API reference: https://ecommerce-api.botble.com/docs
- Support tickets: https://botble.ticksy.com
- Email: contact@botble.com
