...
namespace Botble\Docs\Services;
...
class Indexer
{
     ...
     * @var string
     */
    protected static $indexName = 'botble';
    ...
    public function __construct(AlgoliaManager $client, CustomParser $markdown, Filesystem $files)
    {
        ...
        $this->index = $client->initIndex(static::$indexName . '_tmp');
    }
    ...
    {
        $document = app(Documentation::class);
        foreach ($document->getVersions('cms') as $version) {
            $this->indexAllDocumentsForVersion($version);
        }

        $this->setSettings();

        $this->client->moveIndex($this->index->getIndexName(), static::$indexName);
    }
    ...
    public function indexDocument($version, $path)
    {
        ...
        foreach ($blocs as $bloc) {
            ...
            if (isset($bloc['hidden']) || (isset($bloc['type']) && in_array($bloc['type'],
                        $excludedBlocTypes)) || $bloc['element']['name'] == 'ul') {
                continue;
            }
